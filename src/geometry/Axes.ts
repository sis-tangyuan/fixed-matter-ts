...
export default class Axes {
    ...
    public static fromVertices(vertices: Vertex[]): Vector[] {
        ...
        for (let i = 0; i < vertices.length; i++) {
            ...
            const normal = new Vector(nextPoint.y.sub(curPoint.y), curPoint.x.sub(nextPoint.x)).normalise();
            const gradient = normal.y.eq(Common.ZERO) ? new Decimal(Infinity) : (normal.x.div(normal.y))
